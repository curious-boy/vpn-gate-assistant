...
if __name__ == '__main__':
    #step1: Fetch VPN list in the web
    os.system('get_html.py')

    available_vpn = ''
    
    #step2: check if the previos vpn host is available
    if os.path.isfile('vpn.txt') :
        fb = open('vpn.txt', 'r')
        n = len(fb.readlines())
        fb.close()
        ...
        for i in range(0, n):
            name =  fb.readline( )
            r =  name[0 : -1]
            percent_lost, mrtt, artt = quiet_ping(r)

            if percent_lost != 100 :
                available_vpn = available_vpn + r + '\n'
        else:
            fb.close()

    #step3: check the latest vpn host
    fb = open('temp.txt', 'r')
    n = len(fb.readlines())
    fb.close()
    ...
    #step4: generated the latest available list
    fileHandle = open('vpn.txt', 'w' )  
    fileHandle.write(available_vpn)
    fileHandle.close()
    os.remove('temp.txt')
